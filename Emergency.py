...
# 版本：v1.0
名字：应急自动化分析脚本
简介：采用python2,主要是通过ssh远程登录实现常见命令查看，所以在使用之前需要输入正确的服务器ip、ssh连接端口、ssh登录用户名、ssh登录密码，
主要实现如下功能:
...
ssh.connect('192.168.5.179', 22, 'root', '123')
...
class check_init():
    def get_init(self):
        try:
            command1 = "ls -lth /etc/init.d | sed -n '2,6p'"
            stdin, stdout, stderr = ssh.exec_command(command1)
            info1 = stdout.read()
...
class Host_Info:
    ...
    def get_ip(self):
        try:
            command1 = "ifconfig -a | grep -w inet | grep -v 127.0.0.1 | awk 'NR==1{print $2}'&&hostname&&uname -a &&date&&cat /etc/issue| awk 'NR==1' "
            stdin, stdout, stderr = ssh.exec_command(command1)
            info=stdout.read().splitlines()
            # print info
            self.ip=info[0]
...
class find_gid():
    def get_gid(self):
        ...
        try:
            ...
            print str(e)
#查看日志文件,判断secure日志是否存在异常，由于日志查看的内容比较多，此处只是对secure日志中ip地址进行分析，判断是否存在国外异常ip归属。注：若使用者需要同时监控多个日志，我已经在系统中装了相关工具，可与我联系使用。
class check_log():
    try:
        def cat_log(self):
